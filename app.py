...
def question(question_id):
    user = get_current_user()
    # it doesn't have to be protected from external users
    db = get_db()
...
def answer(question_id):
    user = get_current_user()
    # protect the route from external users
    if not user:
        return redirect(url_for('login'))
    # check if user is expert or not to access this route
    if user['expert'] == 0:
        return redirect(url_for('index'))
...
def users():
    ...
    if not user:
        return redirect(url_for('login'))
    # check if user is admin or not to access this route
    if user['admin'] == 0:
        return redirect(url_for('index'))
